                                    ...
                                    <table class="table-fixed w-full">
                                        ...
                                        @forelse($tasks->pending as $task)
                                            <tr class="bg-white hover:bg-gray-100 text-gray-700 border-b">
                                                <td class="text-center text-sm font-semibold tracking-wide text-gray-500">
                                                    {{ $task->title }}
                                                </td>
                                                <td class="text-center text-xs font-semibold tracking-wide text-gray-500">
                                                    {{ $task->priority->title }}
                                                </td>
                                                <td class="text-center text-xs font-semibold tracking-wide text-gray-500">
                                                   {{ $task->category->title }}
                                    ...
                                    @forelse($tasks->in_progress as $task)
                                        @if($task->status_id === 2)
                                            <div class="bg-white dark:bg-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 p-2 rounded mt-1 border-b border-gray-100 dark:border-gray-900 cursor-pointer">
                                                {{ $task->title }} - <label class="text-xs text-gray-600">{{ $task->priority->title }}</label>
                            ...
                            <div class="rounded bg-gray-200 dark:bg-gray-800 p-3">
                                ...
                                <div class="text-sm text-black dark:text-gray-50 mt-2">
                                    @foreach($tasks->completed as $task)
                                        ...
                                        @endif
                                    @endforeach
                                    @if($tasks->completed->isNotEmpty() && $tasks->completed->count() > 8)
                                        <div class="w-full px-4 py-3 text-xs text-gray-500 border-t bg-gray-50 sm:grid-cols-9">
